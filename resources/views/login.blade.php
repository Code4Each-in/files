...
<style>
  * {
  ...
  .overlay {
    position: relative;
    color: #fff;
    background: linear-gradient(to left, #3c7474, #3c7474);
    left: -100%;
    height: 100%;
    width: 200%;
  ...
  #main_id .overlay-right {
    ...
    transform: translateX(0);
    transition: transform 0.6s ease-in-out;
    background: linear-gradient(to right, #3c7474, #3c7474);
  }
  ...
  <div class="container" id="main">
    ...
    <div class="sign-in">
      <form id="signInForm" novalidate>
        ...
        <input type="hidden" name="fileId" value="{{ request()->query('fileId') }}">
        <div class="form-group">
    ...
    $(document).ready(function() {
      ...
      $('#signInButton').on('click', function() {
        ...
        $.ajax({
          ...
          data: formData,
          success: function(response) {
            if (response.user.hasFileId === true) {
              window.location.href = `{{ route('home') }}?fileId=${response.user.fileId}`;
            } else {
